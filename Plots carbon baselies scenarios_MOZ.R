d <- read_excel("/Users/danic/OneDrive - Indiana University/Research/Data collected/Baselines comparaisons/Baselines cores and FVS_MOZ.xlsx")
...
####BASIC PLOT
plot(d$YEAR, d$`Stand total (ton C/ha)_CORES`)
plot(d$Year...6, d$`5 year average`, type = "l", ylim = c(0, 6), xlim = c(1950,2024))
...
df <- data.frame(
  Year = d$YEAR,
  Year2 = d$Year...6,
  Avg_MMS = d$`5 year average`,
  avg_core = d$`Stand total (ton C/ha)_CORES`
)
...
ggplot(df) +
  ...
  geom_line(aes(x = Year, y = avg_core, color = "Tree core (ton C/ha)")) +
  geom_line(data = new, aes(x = Year, y = Avg_MMS, color = "FVS projection")) +
  #geom_smooth(aes(x = Year, y = NEE..MT.C.Ha.year._MMS, color = "NEE (MT C/Ha year)_MMS"), method = "lm", se = FALSE) +
  geom_smooth(aes(x = Year, y = avg_core, color = "Tree core (ton C/ha)"), method = "lm", se = FALSE) +
  geom_smooth(data = new, aes(x = Year, y = Avg_MMS, color = "FVS projection"), method = "lm", se = FALSE) +
  ylim(0, 5) +
  xlim(1940, 2025) +
  labs(x = "Year", y = "Carbon uptake (MT C/Ha year)", color = "Variable") +
  scale_color_manual(values = c("Tree core (ton C/ha)" = "red", "FVS projection" = "green")) +
  ggtitle("MMSF Carbon baselines")

####MOZ
ggplot(df) +
  geom_line(aes(x = Year, y = avg_core, color = "Tree cores (ton C/ha)")) +
  geom_line(aes(x = Year, y = Avg_MMS, color = "FVS projection")) +
  ylim(0, 4) +
  xlim(1940, 2025) +
  ...
  ggtitle("MOZ Change in Biomass")

ggplot(df) +
  geom_line(aes(x = Year, y = avg_core, color = "Tree cores")) +
  geom_line(aes(x = Year2, y = Avg_MMS, color = "FVS")) +
  ylim(0, 3) +
  xlim(1940, 2025) +
  labs(x = "Year", y = "Change in biomass (MT C/Ha year)", color = "Biometric approach") +
  scale_color_manual(values = c("Tree cores" = "green4", "FVS" = "red4")) +
  ggtitle("US_MOZ Change in Biomass")
#####
